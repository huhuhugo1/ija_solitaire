...
import factory.AbstractFactorySolitaire;
import factory.FactoryKlondike;
...
public class HomeWork2Test {
   ...
   public void testGame() {
      Game game = new Game();
      for (int i = 0; i < 7; i++)
         Assert.assertEquals(i + 1, game.board.workingPacks[i].size());

      for (int i = 0; i < 4; i++)
         Assert.assertEquals(0, game.board.targetPacks[i].size());

      Assert.assertEquals(24, game.board.sourcePack.size());
      Assert.assertEquals(0, game.board.putDownPack.size());
      game.nextCard();
      game.nextCard();
      game.nextCard();
      Assert.assertEquals(21, game.board.sourcePack.size());
      Assert.assertEquals(3, game.board.putDownPack.size());
   }   
}
